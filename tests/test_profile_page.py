...
class TestProfilePage:
    ...
    def test_go_to_profile(self, driver):
        ...
        assert Urls.PROFILE in current_url, f"Ожидался URL профиля, получен: {current_url}"
    ...
    def test_logout(self, driver):
        ...
        assert auth_page.is_auth_page_opened(), "Кнопка «Войти» не отображается после выхода"

        current_url = auth_page.get_current_url()
        assert Urls.LOGIN in current_url, f"Ожидался переход на страницу логина, получен: {current_url}"
    ...
    def test_go_to_order_history_from_profile(self, driver):
        ...
        assert profile_page.is_order_history_page_opened(), "Страница истории заказов не открылась"
